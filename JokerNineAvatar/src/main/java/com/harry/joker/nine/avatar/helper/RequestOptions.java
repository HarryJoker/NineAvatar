...
public abstract class RequestOptions<T> {
    ...
    public void apply(final Object tag) {
        ...
        if (urls != null) {
            JokerLog.d(this.getClass().getSimpleName() + "load nineAvatarUrls from Cache,  tag:" + tag + ", cache:" + urls);
        ...
        //取远程
        JokerLog.d(this.getClass().getSimpleName() + "，Async NineAvatarUrls from Remote, tag:" + tag);
    ...
    private String[] loadUrlsFromCache(Object key) {
        return RemoteCacheHelper.getInstance().getUrlsFromCache(key);
    }
